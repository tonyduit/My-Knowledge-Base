...
执行了helm repo add操作后，会在`~/.config/helm/repositories.yaml`文件中添加一条仓库记录，`helm repo list`就是通过读取这个文件来获取具体仓库信息的
...
- caFile: ""
  ...
  pass_credentials_all: false
  password: ""
  url: https://matrixorigin.github.io/observability-charts
  username: ""
```
...
```shell
➜ helm install stable/mysql --generate-name [-n NAMESPACE]
...
```shell
➜ helm uninstall mysql-1575619811 [-n NAMESPACE]
release "mysql-1575619811" uninstalled

➜ kubectl get all -l release=mysql-1575619811
No resources found.

➜ helm status mysql-1575619811 [-n NAMESPACE]
...
```shell
➜ helm get values mysql [-n NAMESPACE]
...
➜ helm history mysql [-n NAMESPACE]
