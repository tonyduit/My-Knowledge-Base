...
这些所有的 values 值都是由模板作者来定义的，Helm 不会也不需要规定这些参数。可以查看 [Kubernetes Charts 项目](https://github.com/helm/charts)去了解更多的 charts 项目的详细内容。
...
### 测试某个values在模板中渲染出来的效果

```sh
# 将结果直接输出
helm template test-app . -f values.yaml \
--set image.repository="registry.cn-hangzhou.aliyuncs.com/mocloud/matrixone-operator" \
--set kruise.manager.image.repository="registry.cn-hangzhou.aliyuncs.com/mocloud/kruise-manager" \
--set backupRestore.image="registry.cn-hangzhou.aliyuncs.com/mocloud/mo-br:nightly-20240120"

# 将结果输出到某个文件中
helm template test-app . -f values.yaml \
--set image.repository="registry.cn-hangzhou.aliyuncs.com/mocloud/matrixone-operator" \
--set kruise.manager.image.repository="registry.cn-hangzhou.aliyuncs.com/mocloud/kruise-manager" \
--set backupRestore.image="registry.cn-hangzhou.aliyuncs.com/mocloud/mo-br:nightly-20240120" > test-app.yaml
```

### 常用 Flags

为了让 `helm template` 更有用，你通常会结合以下 flags：

- -f, --values FILE: 指定一个或多个`values.yaml`文件。可以多次使用。

  ```bash
  helm template my-release ./my-chart -f my-values.yaml -f another-values.yaml
  ```

- --set KEY=VALUE: 在命令行上直接设置一个值，会覆盖`values.yaml`中的配置。

  ```bash
  helm template my-release ./my-chart --set replicaCount=3 --set image.tag=latest
  ```

- --show-only PATH: 只渲染指定路径的模板文件。这对于调试单个模板文件非常有用。

  ```bash
  # 只渲染 templates/deployment.yaml 文件
  helm template my-release ./my-chart --show-only templates/deployment.yaml
  ```

- `--dry-run`: 这个 flag 本身不直接用于 `helm template`，但它是 `helm install` 和 `helm upgrade` 命令中的一个选项，效果类似。`helm install --dry-run --debug` 会执行一个模拟安装，并在终端打印出渲染后的清单。

- `--debug`: 与 `helm template` 一起使用时，会输出更多调试信息，包括原始的、未渲染的模板内容。
...
由于 Kubernetes 中的大多数对象不同，CRDs 是全局安装的，所以 Helm 在管理 CRD 的时候比较谨慎，会有一些限制：
