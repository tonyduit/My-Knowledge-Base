...
**注意：`SetConfigName("config")`和`SetConfigType("yaml")`并不等于精确匹配`config.yaml`，如果同时有两个文件`config.yaml`和`config.json`依然会报错，想精确匹配要用`SetConfigFile("./config.yaml")`来指定完整的配置文件名**
...
```go
// 如果没有指定类型，他会按照默认的格式搜索顺序去寻找配置文件
SupportedExts = []string{"json", "toml", "yaml", "yml", "properties", "props", "prop", "hcl", "tfvars", "dotenv", "env", "ini"}

// 可以使用viper.SetConfigType("yaml")来确定配置文件的格式
```
...
Viper提供了一种机制来确保ENV变量是唯一的。通过使用`SetEnvPrefix`，你可以告诉Viper在读取环境变量时使用前缀。`BindEnv`和`AutomaticEnv`都将使用这个前缀。
...
**注意** 当我们需要将viper读取的配置反序列化到我们定义的结构体变量中时，一定要使用`mapstructure`tag哦！
