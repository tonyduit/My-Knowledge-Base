...
如果你使用的是gin框架，那么推荐使用[github.com/gin-contrib/pprof](https://github.com/gin-contrib/pprof)，在路由文件中添加这行代码来注册pprof相关路由。
...
![debug/pprof](Go_pprof%E6%80%A7%E8%83%BD%E8%B0%83%E4%BC%98.assets/pprof2.png)
...
**`/debug/pprof/heap` (In-use 视角)：**

- **统计的是：** 程序**当前时刻**存活在内存中的对象（采样数据）。
- **作用：** 它是排查内存**泄漏**的终极工具。如果一个内存块在当前一直没被释放，它就会出现在这里。
- **为什么默认用它：** 因为泄漏通常是指“那些该死还没死掉的对象”，`heap` 路由直接展示的就是这部分。

**`/debug/pprof/allocs` (Cumulative 视角)：**

- **统计的是：** 自程序启动以来，**所有分配过**内存的堆栈。
- **作用：** 它包含了那些已经分配、但后来被 GC 回收掉的内存。
- **为什么一般不用它查泄漏：** 即使你的内存没泄漏，如果你的程序每秒产生大量临时对象（比如高频的 `json.Unmarshal`），`allocs` 的数值会非常惊人，这会干扰你对“内存占用高”的判断。它主要用于优化**分配频率（GC 压力）**。

**`/debug/pprof/goroutine` (堆栈采样视角)：**

- **统计的是：** 当前存活的 Goroutine 的**调用栈**。
- **为什么用它排查栈内存：** 确实，它能看到每个 Goroutine 的堆栈路径，这能帮你发现哪个逻辑导致了成千上万的 Goroutine 阻塞，从而间接推断栈内存消耗。
- **局限性：** 它展示的是**“谁在运行”**，而不是**“谁在消耗字节”**。它不能告诉你“这 10 万个协程一共占用了多少 MB 内存”，它只能告诉你“这里有 10 万个协程卡在 `net.Read` 上”。
...
```bash
go tool pprof cpu.pprof # 或者通过 go tool pprof http://127.0.0.1/debug/pprof/profile 直接获取
```
...
- flat（Flat Time）：当前函数占用CPU的耗时（平层耗时）
- flat%：当前函数占用CPU的耗时百分比
- sum%：函数占用CPU的累计耗时百分比（**第N个sum% = 前N个flat%的总和**）
- cum（Cumulative Time）：当前函数加上调用当前函数的函数占用CPU的总耗时
...
![CPU占比图](Go_pprof%E6%80%A7%E8%83%BD%E8%B0%83%E4%BC%98.assets/cpu_pprof.png)

关于图形的说明： 每个框代表一个函数，理论上框的越大表示占用的CPU资源越多。 方框之间的线条代表函数之间的调用关系。 线条上的数字表示函数调用的次数。 方框中的第一行数字表示当前函数占用CPU的百分比（flat%），第二行数字表示当前函数加上调用当前函数的函数累计占用CPU的百分比（cum%）。

除了分析CPU性能数据，pprof也支持分析内存性能数据。比如，使用下面的命令分析http服务的heap性能数据，查看当前程序的内存占用以及热点内存对象使用的情况。

```bash
# 查看内存占用数据
go tool pprof -inuse_space http://127.0.0.1:8080/debug/pprof/heap # 统计当前正在使用的内存大小
go tool pprof -inuse_objects http://127.0.0.1:8080/debug/pprof/heap # 统计当前正在使用的对象个数
# 查看临时内存分配数据
go tool pprof -alloc_space http://127.0.0.1:8080/debug/pprof/heap # 程序运行至今累计分配的内存大小
go tool pprof -alloc_objects http://127.0.0.1:8080/debug/pprof/heap # 程序运行至今累计分配的对象个数
```

## go-torch和火焰图

火焰图（Flame Graph）是 Bredan Gregg 创建的一种性能分析图表，因为它的样子近似🔥而得名。上面的 profiling 结果也可以转换成火焰图，如果对火焰图比较了解可以手动来操作，不过这里我们要介绍一个工具：`go-torch`。这是 uber 开源的一个工具，可以直接读取 golang profiling 数据，并生成一个火焰图的 svg 文件。
...
![火焰图](Go_pprof%E6%80%A7%E8%83%BD%E8%B0%83%E4%BC%98.assets/pprof3.png)
...
只需要在程序的开始和结束的地方插入runtime/pprof的注册逻辑即可
...
这里就会进入一个命令行，可以输入不同的命令例如top3（查看前3个CPU占用率最高的函数）、list ${函数名}（详细分析函数的问题）、web（通过svg图的方式查看程序中详细的CPU占用情况，需要graphviz）

**对于服务型应用（以Gin为例）**：

只需要在路由中注册`github.com/gin-contrib/pprof`即可
...
这里也会进入一个命令行，可以输入不同的命令例如top3（查看前3个CPU占用率最高的函数）、list ${函数名}（详细分析函数的问题）、web（通过svg图的方式查看程序中详细的CPU占用情况，需要graphviz）

**火焰图与go-torch**

go-torch是一个封装了pprof的工具，可以直接读取 golang profiling 数据，并生成一个火焰图的 svg 文件。
...
```bash
go-torch -u http://127.0.0.1:8080 -t 30 # 路由默认为 /debug/pprof/profile
```

30秒之后终端会初夏如下提示：`Writing svg to torch.svg`

然后我们使用浏览器打开`torch.svg`就能看到火焰图了。
