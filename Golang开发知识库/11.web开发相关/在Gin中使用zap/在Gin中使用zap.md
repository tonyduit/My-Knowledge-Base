...
func InitLogger(cfg *config.LogConfig) (err error) {
	...
	encoder := getEncoder()
	var l = new(zapcore.Level) // 先创建类型为 zapcore.Level（一个枚举型的级别）的变量 l ，zapcore.Level 实现了 Enabled(Level) bool，因此它本身就是一个 LevelEnabler。
	err = l.UnmarshalText([]byte(cfg.Level)) // cfg.Level是字符串，先把字符串解析成 zapcore.Level 类型的值，然后再赋值给 l
	if err != nil {
		return
	}
	core := zapcore.NewCore(encoder, writeSyncer, l) // l 再作为 LevelEnabler 类型变量传入。
