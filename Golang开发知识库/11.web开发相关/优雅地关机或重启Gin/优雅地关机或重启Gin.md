...
- 当你调用 `signal.Notify(quit, syscall.SIGINT, syscall.SIGTERM)` 时，Go 的运行时会启动一个**内部的、专用的 goroutine**（或复用已有信号处理线程），负责监听操作系统发来的信号。
- 如果没有收到信号，`<-quit`就会一直阻塞住
- 一旦收到 `SIGINT`（如 Ctrl+C）或 `SIGTERM`（如 `kill <pid>`），**这个内部 goroutine 会自动向 `quit` channel 发送对应的 `os.Signal` 值**，然后`<-quit`就会结束阻塞，继续执行后续的关机代码。
- 基于`context.WithTimeout()`设置一个5秒超时的上下文，然后通过defer语句来实现5秒后强制关机。
- 最后调用http.Sever内置的优雅关机方法`Shutdown()`，并将5秒超时的上下文传递给他，实现5秒内的优雅关机。
