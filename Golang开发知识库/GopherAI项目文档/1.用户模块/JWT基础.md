...
### 1. [JSON Web Token](https://zhida.zhihu.com/search?content_id=107448780&content_type=Article&match_order=1&q=JSON+Web+Token&zhida_source=entity)是什么

> JSON Web Token (JWT)是一个开放标准([RFC 7519](https://zhida.zhihu.com/search?content_id=107448780&content_type=Article&match_order=1&q=RFC+7519&zhida_source=entity))，它定义了一种紧凑的、自包含的方式，用于作为JSON对象在各方之间安全地传输信息。该信息可以被验证和信任，因为它是数字签名的。
...
![img](JWT%E5%9F%BA%E7%A1%80.assets/v2-2d4a2985706ff9610ad1466b66ba09ad_1440w.jpg)

### 4.JWT是如何工作的

在认证的时候，当用户用他们的凭证成功登录以后，一个JSON Web Token将会被返回。此后，token就是用户凭证了，你必须非常小心以防止出现安全问题。一般而言，你保存令牌的时间不应该超过你所需要它的时间。

无论何时用户想要访问受保护的路由或者资源的时候，用户代理（通常是浏览器）都应该带上JWT，典型的，通常放在[Authorization header](https://zhida.zhihu.com/search?content_id=107448780&content_type=Article&match_order=1&q=Authorization+header&zhida_source=entity)中，用[Bearer schema](https://zhida.zhihu.com/search?content_id=107448780&content_type=Article&match_order=1&q=Bearer+schema&zhida_source=entity)。
...
- CSRF : 用户很容易受到[CSRF攻击](https://zhida.zhihu.com/search?content_id=107448780&content_type=Article&match_order=1&q=CSRF攻击&zhida_source=entity)。
...
![img](JWT%E5%9F%BA%E7%A1%80.assets/v2-b7eeaf69feb5cbbadea7e0c73056da7a_1440w.jpg)
...
![img](JWT%E5%9F%BA%E7%A1%80.assets/v2-7789057d5744891fdf3366d7887ab6e8_1440w.jpg)
...
- [OAuth2](https://zhida.zhihu.com/search?content_id=107448780&content_type=Article&match_order=1&q=OAuth2&zhida_source=entity)是一种授权框架 ，JWT是一种认证协议 
