...
**1. 传统前缀树 (Trie / Prefix Tree)**

```
注册路由：/user、/users、/user/profile
...
- 每个**字符**占用一个节点
- 路径 `/user/profile` 需要 12 个节点
- 内存开销大，树深度深

**2. 基数树 (Radix Tree / Compressed Trie)**

```
注册路由：/user、/users、/user/profile

Radix Tree 结构（压缩公共前缀）：
...
![img](2.Gin%E6%A1%86%E6%9E%B6%E6%BA%90%E7%A0%81%E8%A7%A3%E6%9E%90.assets/radix_tree.png)
...
[ 路由层级检查 ]
     │
     ├── /users  (Priority: 50)  <-- [最长：优先级最高]
     │    │                          理由：子孙节点最多，命中概率理论最高
     │    └── (50个子路由...)         动作：匹配时最先对比这个字符串
     │
     ├── /posts  (Priority: 10)  <-- [次长：优先级第二]
     │    │                          理由：规模中等
     │    └── (10个子路由...)         动作：如果没匹配到 /users，接着对比这个
     │
     ├── /admin  (Priority: 1)   <-- [较短：优先级第三]
     │    └── (1个子路由)             
     │
     └── /info   (Priority: 1)   <-- [较短：优先级第四]
                                     理由：和 /admin 权重一样，但注册代码写在后面
```
...
![addroute](2.Gin%E6%A1%86%E6%9E%B6%E6%BA%90%E7%A0%81%E8%A7%A3%E6%9E%90.assets/addroute.gif)
...
![gin_middleware1](2.Gin%E6%A1%86%E6%9E%B6%E6%BA%90%E7%A0%81%E8%A7%A3%E6%9E%90.assets/gin_middleware1.png)

我们可以在中间件函数中通过再次调用`c.Next()`实现嵌套调用（func1中调用func2；func2中调用func3），

![gin_middleware2](2.Gin%E6%A1%86%E6%9E%B6%E6%BA%90%E7%A0%81%E8%A7%A3%E6%9E%90.assets/gin_middleware2.png)
...
![gin_middleware3](2.Gin%E6%A1%86%E6%9E%B6%E6%BA%90%E7%A0%81%E8%A7%A3%E6%9E%90.assets/gin_middleware3.png)
