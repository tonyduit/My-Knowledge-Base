...
// Binding from JSON
type Login struct {
	User     string `form:"user" json:"user" binding:"required"` // form 标签同时用于 Form 表单 和 QueryString 的绑定。
...
func main() {
	...
	// 可以通过下面的方式修改
	// router.MaxMultipartMemory = 8 << 20  // 8 MiB也可以写为8388608
...
r.GET("/test", func(c *gin.Context) {
    // 指定重定向的URL
    c.Request.URL.Path = "/test2" // 把请求的URL修改
...
#### 如果中间件不需要自定义参数，可以不嵌套吗？

可以！比如：

```go
// 直接定义一个 func(*gin.Context)
...
// 使用
r.Use(SimpleLogger) // 注意：没有括号！因为 SimpleLogger 本身就是 func(*Context)
```

✅ 这完全合法！而且**没有闭包嵌套**。

但问题来了：**如果我想控制日志级别呢？比如只记录 error 级别以上的请求？**

**当中间件需要“配置参数”时，就必须用闭包！**

假设你想这样使用：

```go
r.Use(LogIfStatusGE(400)) // 只记录状态码 >=400 的请求
r.Use(LogIfStatusGE(500)) // 只记录 5xx 错误
```

你怎么实现 `LogIfStatusGE`？

**❌ 错误尝试：直接写成 func(\*Context)**

```go
func LogIfStatusGE(c *gin.Context, threshold int) { // ← 多了一个参数！
    // ...
}
```

但 Gin 调用时只会传 `c`，不会传 `threshold`！**类型不匹配，编译都过不了**。

**正确做法：用闭包“固化”配置**

```go
// 外层函数：接收配置参数
func LogIfStatusGE(threshold int) gin.HandlerFunc {
    // 返回一个 func(*gin.Context)，它“记住”了 threshold
    return func(c *gin.Context) {
        c.Next()
        if c.Writer.Status() >= threshold {
...
1. `LogIfStatusGE(400)` **被立即调用**，返回一个 `func(*gin.Context)`
2. 这个返回的函数**捕获了 `threshold = 400`**
3. Gin 后续调用这个函数时，只需传 `c`，它内部就能用 `threshold`

**类比：工厂生产工具**

- `GinRecovery(true)` 就像 **“生产一个带堆栈打印功能的 recovery 工具”**
- `GinRecovery(false)` 就像 **“生产一个不带堆栈打印的 recovery 工具”**
- Gin 在请求来时，**使用这个工具去处理请求（传入 context）**

工具（中间件）本身只需要知道怎么处理请求（`*Context`），而**怎么制造工具**（是否带堆栈）是在启动时决定的。

**总结：外层函数用于“定制中间件的行为（通过参数）”，内层函数用于“执行中间件的逻辑（通过 context）”。两者分工明确，缺一不可。**
...
func func4(c *gin.Context) {
	fmt.Println("func4 start")
}

func func5(c *gin.Context) {
	fmt.Println("func5 start")
}
...
func main() {
	...
	shopGroup := r.Group("/shop", func1, func2) // 相当于给这个路由组的所有子路由都配置了func1和func2中间件
	shopGroup.Use(func3)
	{
...
| 特性     | 参数中按顺序写多个函数               | 中间件内使用 c.Next()                      |
| -------- | ------------------------------------ | ------------------------------------------ |
| 执行时机 | 顺序执行，前一个完成后才执行下一个   | 可以在当前中间件内控制后续中间件的执行时机 |
| 前后逻辑 | 只能在前一个函数执行后才能执行下一个 | 可以在 `c.Next()` 前后都执行逻辑           |
| 控制权   | 被动顺序执行                         | 主动控制执行流程                           |
| 典型用途 | 简单中间件链                         | 需要"前后处理"的中间件（如计时、日志）     |

**c.Abort()**的作用：

它底层会将遍历中间件函数链条`[]HandlerFunc `的for循环的当前索引变为`math.MaxInt8`=63，以实现让循环结束的效果，然后中间件就会由内向外逐个返回了。
