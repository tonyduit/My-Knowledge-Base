...
defer 语句会在函数退出时（无论正常退出还是异常退出）执行，即使程序因 `panic` 终止，`defer db.Close()` 依然会被调用。
...
-- 插入一条测试数据
insert into user (name, age) values("qimi", 18);
```
...
func queryRowDemo() {
	...
	err := db.QueryRow(sqlStr, 1).Scan(&u.id, &u.name, &u.age) // 传入id的值为1
	if err != nil {
