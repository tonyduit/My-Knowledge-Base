...
![Kafka-Topic-1](1.Kafka%E5%85%A5%E9%97%A8.assets/1721b74dbaf8bfaftplv-t2oaga2asx-jj-mark3024000q75.webp)
...
![Kafka-Topic-2](1.Kafka%E5%85%A5%E9%97%A8.assets/1721b745dd83d3d0tplv-t2oaga2asx-jj-mark3024000q75.webp)

一个分区只能被同一个消费者群组里面的一个消费者读取，但可以被不同消费者群组中所组成的多个消费者共同读取。多个消费者群组中消费者共同读取同一个主题时，彼此之间互不影响，因为每个**消费者组**，Kafka 都会**单独保存一份该分区的 offset 偏移量**。

![Kafka-ConsumerGroup](1.Kafka%E5%85%A5%E9%97%A8.assets/1721b745e2467db0tplv-t2oaga2asx-jj-mark3024000q75.webp)
...
![Kafka-Brokers](1.Kafka%E5%85%A5%E9%97%A8.assets/1721b75203cc51datplv-t2oaga2asx-jj-mark3024000q75.webp)
...
为保证 producer 发送的数据能到达指定的 topic，topic 的每个 partition 收到 producer 发送的数据后，都需要向 producer 发送 ack（acknowledgement 确认收到），如果 producer 收到 ack，就会进行下一轮的发送，否则重新发送数据。

![Kafka-ack](1.Kafka%E5%85%A5%E9%97%A8.assets/1721b745e31d123btplv-t2oaga2asx-jj-mark3024000q75.webp)
...
| 全部完成同步，才发送 ack     | 选举新的 leader 时，容忍 n 台节点的故障，只需要 n+1 个副本 | 延迟高                                                    |
...
![数据一致性问题](1.Kafka%E5%85%A5%E9%97%A8.assets/1721b745e5fb7acctplv-t2oaga2asx-jj-mark3024000q75.webp)
...
> 每一个「消费组 + 主题 + 分区」，单独一条 offset 记录。

这就是为什么：

- **不同消费组**：各自有独立 offset，互不干扰
- **同一个消费组**：组内所有消费者**共用同一份 offset**
...
![分区和副本](1.Kafka%E5%85%A5%E9%97%A8.assets/1721b74db144d195tplv-t2oaga2asx-jj-mark3024000q75.webp)
...
![元数据请求机制](1.Kafka%E5%85%A5%E9%97%A8.assets/1721b74db183b272tplv-t2oaga2asx-jj-mark3024000q75.webp)

**数据可见性**

需要注意的是，并不是所有保存在分区首领上的数据都可以被客户端读取到，为了保证数据一致性，只有被所有同步副本 (ISR 中所有副本) 都保存了的数据才能被客户端读取到。

![数据可见性](1.Kafka%E5%85%A5%E9%97%A8.assets/1721b74611b33e7etplv-t2oaga2asx-jj-mark3024000q75.webp)
